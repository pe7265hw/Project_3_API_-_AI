...
A successful execution will display two to three recipes including a picture, facts about the recipe, ingredient and instructions. If there are no results from the API another page will explain 
this to the user and if Gemini could not return any results based on the users input a separate page will tell the user their response could not be processed. If the user uses more than 50 API 
